...
cd ..
PathToMilkyWayAtHomeClientFolder='/mnt/c/Users/guoy18/change'
if $rebuild
then
    ...
    mkdir build
    cd build
    cmake -DCMAKE_BUILD_TYPE=Release -DNBODY_DEV_OPTIONS=ON -DNBODY_GL=OFF -DBOINC_APPLICATION=OFF -DSEPARATION=OFF -DDOUBLEPREC=ON -DNBODY_OPENMP=ON -DNBODY_OPENCL=OFF -DAUTODIFF=OFF -DAUTODIFF_LOG=OFF $PathToMilkyWayAtHomeClientFolder/milkywayathome_client/
    make -j 
    cd ..
fi
